...
uint8_t src[DMA_SIZE] __attribute__((aligned(32)));  // 源数据
uint8_t dst[DMA_SIZE] __attribute__((aligned(32)));  // 目标数据
...
void CounterHandler(void *InstancePtr) {
  ...
  SetupDebugBits(true, DEBUG_BIT_FIFO_WRITE_START);
  SetupDebugBits(false, DEBUG_BIT_FIFO_READ_START);
  SetupDebugBits(false, DEBUG_BIT_FIFO_WRITE_START);
...
int SystemInit() {
  ...
  check(SetupIntrSystem(&intc, IRQN_COUNTER_TRIGGER, CounterHandler, 8 * 10,
                        NULL));
  check(
      SetupIntrSystem(&intc, IRQN_ADC_FIFO_FULL, CounterHandler, 8 * 11, NULL));
...
int main() {
  ...
  while (delay-- && !intrFlag) SmallDelay();
  if (!delay) {
    Log("Failed!");
    // return 1;
  } else {
